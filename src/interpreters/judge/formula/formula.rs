...
impl Interpreter {
    ...
    fn formula_push(
        ...
        match ast {
            ...
            Syntax::Call(call) => {
                let inner =
                    self.serch_fun(call.get_name())
                        .ok_or(result::Error::InterpreterError(format!(
                            "{} function not found",
                            call.get_name()
                        )))?;

                let function_return = self.function_run(&inner, call, None)?.ok_or(
                    result::Error::InterpreterError(format!(
                        "{} not a return value",
                        call.get_name()
                    )),
                )?;
    ...
    fn formula_object<T: Node>(
        ...
        match bin_innner_node {
            ...
            Syntax::Call(call) => {
                let serched_function = inner.serch_functions(call.get_name()).ok_or(
                    result::Error::InterpreterError(format!(
